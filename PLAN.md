...
- Form descriptions now expose widget template, option-template, checked-state,
  ...
  target IDs, and `show_hidden_initial` hidden initial widget/ID metadata for
  richer frontend rendering.
...
- Semantic OpenAPI contract tests now cover core site/model route operation IDs, tags, security, request body schemas, generated JSON mutation examples, success response schemas, and typed error response maps.
...
- Add per-row inline formset metadata for Django form prefixes, management-form
  field names, and prefixed bound-field IDs/names.
