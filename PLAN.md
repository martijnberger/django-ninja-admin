...
- Custom actions can now declare Pydantic/Ninja input and response schemas through `@action(input_schema=..., response_schema=...)`; input schemas are validated before dispatch and both sides are exposed in OpenAPI.
...
- Action payload schemas now advertise action names plus optional custom input/response schema unions; deeper action-specific discriminated contracts still need work.
...
- OpenAPI generation works and now has semantic contract coverage for core model routes and custom action input/response schemas, but it does not yet expose every action-specific discriminated variant or the same richness upstream provided through drf-spectacular hooks.
