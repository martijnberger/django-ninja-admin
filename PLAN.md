...
- Admin extensibility is still young: custom view routing, direct/decorator route registration, route metadata/auth overrides, named response-schema coverage, site/route-level multi-auth coverage, and display metadata exist, but deeper override-hook parity still needs work.
