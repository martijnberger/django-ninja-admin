...
- System checks now cover common invalid configurations, many-to-many `list_display` mistakes, `list_editable` form-layout conflicts, duplicate `readonly_fields`, `list_select_related` mistakes, autocomplete target registration/searchability, and relation/widget option conflicts, but they do not yet match Django's complete check coverage or IDs.
