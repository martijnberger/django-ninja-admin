...
- Form descriptions now expose widget template, option-template, checked-state,
  ...
  states, IDs, and attrs, `ClearableFileInput` clear-checkbox names/IDs/labels,
  inline formset prefixes, management-form fields, empty-form template rows,
  per-row form metadata, and `show_hidden_initial` hidden initial widget/ID
  metadata for richer frontend rendering.
