...
- Changelist behavior now supports last-page pagination, row/result indexes, page-result/range metadata, page-choice metadata, pagination/show-all query strings, filter-state/clear-all metadata, facet toggle links, lowest-useful initial date hierarchy levels, and preservation of unrelated lookup params when resetting stale page/order links, but is still not fully equivalent to upstream `ChangeList`; remaining query-string edge cases, richer result rendering metadata, list-editable formset parity, additional date hierarchy edge cases, and broader N+1 hardening still need work.
