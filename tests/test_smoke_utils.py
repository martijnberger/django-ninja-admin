...
def test_resolve_prebuilt_wheel_returns_none_without_env(monkeypatch):
    monkeypatch.delenv("DJANGO_VEO_ADMIN_API_WHEEL", raising=False)

    assert resolve_prebuilt_wheel() is None


def test_resolve_prebuilt_wheel_accepts_file(monkeypatch, tmp_path):
    wheel = tmp_path / "django_veo_admin_api-1.2.3-py3-none-any.whl"
    wheel.write_text("wheel", encoding="utf-8")
    monkeypatch.setenv("DJANGO_VEO_ADMIN_API_WHEEL", str(wheel))

    assert resolve_prebuilt_wheel() == wheel.resolve()


def test_resolve_prebuilt_wheel_accepts_directory(monkeypatch, tmp_path):
    older = tmp_path / "django_veo_admin_api-1.2.2-py3-none-any.whl"
    newer = tmp_path / "django_veo_admin_api-1.2.3-py3-none-any.whl"
    older.write_text("older", encoding="utf-8")
    newer.write_text("newer", encoding="utf-8")
    monkeypatch.setenv("DJANGO_VEO_ADMIN_API_WHEEL", str(tmp_path))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "django_veo_admin_api-1.2.3.tar.gz",
    ],
)
def test_resolve_prebuilt_wheel_rejects_invalid_values(monkeypatch, tmp_path, path):
    ...
    monkeypatch.setenv("DJANGO_VEO_ADMIN_API_WHEEL", str(candidate))
...
def test_resolve_prebuilt_wheel_rejects_empty_directory(monkeypatch, tmp_path):
    monkeypatch.setenv("DJANGO_VEO_ADMIN_API_WHEEL", str(Path(tmp_path)))
