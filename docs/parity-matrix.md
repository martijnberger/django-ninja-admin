...
| Behavior | Status | Evidence | Remaining Work |
| --- | --- | --- | --- |
| Dynamic output schemas | partial | `BaseAdmin.get_output_schema`, non-null typed primary keys, blank/non-null model fields, explicit JSON value schemas, decimal precision constraints, bounded numeric model-field constraints including strictest min/max validator collapse for relation target schemas, zero-offset step validators, string length validators, email/URL/IP format metadata, base64 binary-field output metadata, model regex/slug patterns, typed file/image/relation/many-to-many/choice fields including constrained many-to-many target items and target-aware examples, Ninja `register_field()` mappings for custom primary keys/relation output IDs/examples, and schema override tests including computed `ModelAdmin` methods | Stable component names and richer custom fields |
