...
| Raw ID/radio/filter-horizontal/prepopulated | partial | Per-field `admin_widget`, structured raw-id lookup metadata, radio orientation, prepopulated source, filter-horizontal/filter-vertical, and conflict-check tests | Full Django widget rendering semantics and more edge cases |
