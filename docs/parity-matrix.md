...
| Admin system checks | partial | `django_ninja_admin/checks.py`, invalid-admin, callable/many-to-many/reverse relation `list_display`, action permission hooks, `list_editable` form-layout and first-column link conflicts, inline count option checks, row-tuple `fields`, duplicate `fields`, fieldset shape/duplicate checks, `fields`/`exclude` item checks, `prepopulated_fields`, `sortable_by`, ordering random-marker/expression checks, field-based `list_filter` tuple/class checks, pagination options, boolean/save/view-on-site options, custom `form_class`, `formfield_overrides`, `list_select_related`, widget-conflict, reverse relation widget, and manual-through m2m widget tests | Match Django check IDs/coverage more closely |
...
| Inline add/change/delete | partial | Formset implementation, typed inline schemas, dynamic inline count metadata, row-indexed server errors, max/delete/duplicate/conflict/unknown-object/rollback/permission/unknown-key/unknown-field/readonly-field tests | Deeper upstream formset edge cases |
...
| Default delete action/actions | partial | `delete_selected`, custom return, empty-selection, invalid selected IDs, select-across, action permission-hook checks, and protected response tests | Additional permission edge coverage |
