...
| File/image fields | partial | FileField output schema/serialization, ImageField typed metadata, current-file form metadata including storage backends without URLs, JSON clear tests, multipart create/update routes, Pydantic validation for multipart JSON parts, and mounted ImageField invalid/valid upload tests with dimension metadata | Deeper storage/widget edge cases |
