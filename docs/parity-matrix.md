...
| Behavior | Status | Evidence | Remaining Work |
| --- | --- | --- | --- |
| Dynamic output schemas | partial | `BaseAdmin.get_output_schema`, non-null typed primary keys, blank/non-null model fields, explicit JSON value schemas, decimal precision constraints, bounded numeric model-field constraints including strictest min/max validator collapse for relation target schemas, zero-offset step validators, string length validators, email/URL/IP format metadata, base64 binary-field output metadata, model regex/slug patterns, typed file/image/relation/many-to-many/choice fields including constrained many-to-many target items and target-aware examples, Ninja `register_field()` mappings for custom primary keys/relation output IDs/examples, and schema override tests including computed `ModelAdmin` methods plus valid examples for common explicit constrained scalar/container override types | Stable component names and richer custom fields |
| Pydantic request envelopes | partial | Per-model create/update/bulk, relation inputs typed from target primary key or `to_field_name` including Ninja `register_field()` custom target fields and target string constraints, unknown parent-field rejection, explicit parent/inline/bulk `form_schema_field_overrides` with validating component and route-level examples, richer JSON-compatible value schemas/UUID/IP/email/URL/duration/date-time-input-format/split-datetime/generic-multivalue/file-path/combo-field/file-image/null-boolean/multiple-choice/typed-choice form-field types, enum-style concrete/grouped/plain-non-JSON-scalar/typed/float/decimal/UUID choices, string/form-validator/numeric-validator/field-regex/validator-regex/numeric/decimal/step constraints including Django-style stripping, disabled parent/inline field optionality, per-inline operation, discriminated action payload variants, and custom action input schemas with OpenAPI tests | Broader edge-case schemas and snapshots |
...
| Raw ID/radio/filter-horizontal/prepopulated | partial | Per-field `admin_widget`, structured raw-id lookup metadata with related changelist URL/query hints and target-field metadata, structured filtered-select metadata including dual-select direction/stacking, related-model identity, and target-field metadata, structured radio metadata, structured prepopulated source metadata, and conflict-check tests | Full Django widget rendering semantics and more edge cases |

## Mutations, Inlines, Delete, And Logs

| Behavior | Status | Evidence | Remaining Work |
| --- | --- | --- | --- |
| Create/update/delete routes | partial | CRUD/history tests plus mounted save-form/save/delete/response hook, declared schemas for custom add/change/delete `Status` responses, typed mutation success response maps, and allowed/bad `_to_field` detail/update/delete tests | More response hook edge cases |
...
| Typed error bodies | partial | Exception handlers, Ninja validation handler, runtime auth-error tests, built-in/model/custom route OpenAPI error maps including conditional `401` responses, concrete `ErrorResponse` OpenAPI examples for validation, permission, protected-delete, and deleted-object tree bodies, semantic `ErrorItem`/`ErrorResponse` component snapshots, and representative runtime body validation for auth, permission, not-found, request-validation, and protected-delete errors | Broader cross-route runtime edge coverage |
...
| Per-model request/response contracts | partial | Output, write payload, generated create/update/bulk/action request examples, generated output/mutation/bulk/inline component examples including self-validating many-to-many/custom-field/output-override examples, typed mutation/bulk response data, typed default action responses, declared custom action/add/change/delete hook response schemas, changelist/list-editing/form/inline response metadata components including `CellMetadata`, normalized fieldset layout schemas, `FieldDescription.attrs` examples, inline operation, multipart JSON part media hints plus required file parts, discriminated action payload/input/response, alternate custom-action success status responses, global action cache invalidation, auth-aware response-map schemas in OpenAPI, semantic OpenAPI diff tooling for release review, and installed-project OpenAPI consumer smoke coverage for list/create/detail/patch/bulk/action/form operations | Broader Phase 6 model-route snapshots |
