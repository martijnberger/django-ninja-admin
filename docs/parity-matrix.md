...
| Behavior | Status | Evidence | Remaining Work |
| --- | --- | --- | --- |
| Dynamic output schemas | partial | `BaseAdmin.get_output_schema`, non-null typed primary keys and blank/non-null model fields, typed file/image/relation/many-to-many/choice fields, Ninja `register_field()` mappings for custom primary keys and relation output IDs, and schema override tests including computed `ModelAdmin` methods | Stable component names and richer custom fields |
