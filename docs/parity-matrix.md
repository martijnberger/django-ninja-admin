...
| `list_select_related` and relation query optimization | partial | `auto_select_related_fields`, direct-FK, relation-path, callable related-ordering query-count tests, explicit `list_prefetch_related` support, and callable many-to-many display query-count tests | More inferred prefetch strategy and broader query-count coverage |
