...
| Ordering/sort links | partial | Sort metadata plus default-ordering and multi-column ordering state/link tests, deterministic primary-key fallback ordering, stale-page reset in generated links, pagination query strings, and preservation of unrelated lookup params such as `price__gte` | Deeper Django query-string semantics |
