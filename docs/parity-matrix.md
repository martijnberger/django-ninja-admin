...
| Form descriptions | partial | Widget, custom-widget, disabled-field, widget templates/subwidgets, `SelectDateWidget`, temporal input formats, form media, validator names/details, error-message, localization, initial values, structured relation/autocomplete, selected relation labels, relation `limit_choices_to`, numeric, decimal/step, flat/grouped choice with typed raw values, file-path/combo-field, readonly display/callable, model-field identity, file/image, m2m, and admin-widget metadata tests | Custom field metadata and advanced widget details |
| File/image fields | partial | FileField output schema/serialization, ImageField typed metadata, current-file form metadata including storage backends without URLs, typed string-or-null JSON write schemas, JSON clear tests for files/images, multipart create/update routes, Pydantic validation for multipart JSON parts, and mounted ImageField invalid/valid upload tests with dimension metadata | Deeper storage/widget edge cases |
...
| Bulk list-editable update | partial | Strict row schema, duplicate-PK rejection, filtered-queryset row limiting, `_to_field` row identity support, structured list-editing row metadata, unchanged-row skip, empty change-log skip, row-indexed validation and permission errors, and all-rows-before-write tests | Remaining changelist formset semantics |
...
| Per-model request/response contracts | partial | Output, write payload, mutation/bulk response, inline operation, multipart JSON part media hints, discriminated action payload/input/response, global action cache invalidation, and auth-aware response-map schemas in OpenAPI | Broader Phase 6 snapshots and examples |
