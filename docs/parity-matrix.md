...
| Lookup validation | partial | `lookup_allowed`, changelist `_to_field` validation/row identity tests, direct `__in`/`__isnull` lookup preparation, local lookup-suffix tests, `limit_choices_to` relation-lookup tests, invalid lookup-key tests, and invalid lookup-value typed error tests | Match remaining Django suspicious lookup edge cases |
