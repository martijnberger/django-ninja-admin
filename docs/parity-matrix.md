...
| File/image fields | partial | FileField output schema/serialization, ImageField typed metadata, current-file form metadata including storage backends without URLs, JSON clear tests, multipart create/update routes, and Pydantic validation for multipart JSON parts | Real image upload validation and deeper storage/widget edge cases |
