...
| Behavior | Status | Evidence | Remaining Work |
| --- | --- | --- | --- |
| Dynamic output schemas | partial | `BaseAdmin.get_output_schema`, non-null typed primary keys, blank/non-null model fields, decimal precision constraints, bounded numeric model-field constraints, email/URL format metadata, base64 binary-field output metadata, typed file/image/relation/many-to-many/choice fields including constrained many-to-many target items, Ninja `register_field()` mappings for custom primary keys and relation output IDs, and schema override tests including computed `ModelAdmin` methods | Stable component names and richer custom fields |
