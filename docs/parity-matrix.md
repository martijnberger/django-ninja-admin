...
| Custom site/model views | partial | `admin_view()`, `get_urls()`, direct and decorator-style `route()`, route tags/descriptions, hidden routes, raw method wrapping, `auth=None`, named response schemas, generated/explicit route operation IDs, custom route tests, and site/route-level auth-sequence route tests | Deeper override-hook parity and upstream fixture comparisons |
