...
| `GET /history` | partial | History route, log model, app/model/object/action filter, model/object-level permission-filter tests, client-controlled page/page-size pagination, per-row model identity/object links, and Django-style change-message text | Broader upstream history fixture comparisons |
| `GET /autocomplete` | partial | Permission-hardened route plus rich pagination metadata, remote `get_paginator()` and `get_search_fields()` hook coverage, object-level remote view-permission result filtering, many-to-many source-field tests, non-PK `ForeignKey(to_field=...)` option IDs, and source-field `limit_choices_to` filtering/form metadata coverage | Additional remote-field edge cases |
