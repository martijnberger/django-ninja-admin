...
| Custom site/model views | partial | `admin_view()`, `get_urls()`, `route()`, route tags/descriptions, hidden routes, raw method wrapping, `auth=None`, custom route tests | More multi-auth and response-schema override coverage |
...
| Pydantic request envelopes | partial | Per-model create/update/bulk, per-inline operation, action-name schemas, and custom action input schemas with OpenAPI tests | Broader custom action response contracts |
...
| Per-model request/response contracts | partial | Output, write payload, inline operation, action payload/input, and response-map schemas in OpenAPI | Custom action response schemas and Phase 6 snapshots |
