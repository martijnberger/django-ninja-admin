...
| `GET /autocomplete` | partial | Permission-hardened route plus rich pagination metadata, remote `get_paginator()` and `get_search_fields()` hook coverage, object-level remote view-permission result filtering, many-to-many source-field tests, and non-PK `ForeignKey(to_field=...)` option IDs | Additional remote-field edge cases |
