...
| `date_hierarchy` | partial | Date hierarchy metadata/filter tests, relation-path date fields, field-type/timezone metadata, active-timezone DateTime bucketing, clear/back navigation query strings, selected day state, and invalid date validation | Additional date edge cases and deeper Django navigation parity |
