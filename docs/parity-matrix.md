...
| Per-model request/response contracts | partial | Output, write payload, generated create/update/bulk/action request examples, generated output/mutation/bulk/inline component examples, mutation/bulk response, changelist/list-editing/form/inline response metadata components, normalized fieldset layout schemas, `FieldDescription.attrs` examples, inline operation, multipart JSON part media hints plus required file parts, discriminated action payload/input/response, alternate custom-action success status responses, global action cache invalidation, and auth-aware response-map schemas in OpenAPI | Broader Phase 6 model-route snapshots |
