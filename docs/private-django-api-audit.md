...
| --- | --- | --- | --- |
| `_get_foreign_key` | `django_veo_admin_api/admins/inline.py`, `django_veo_admin_api/checks.py`, `django_veo_admin_api/core/operations/forms.py`, `django_veo_admin_api/sites.py` | Match Django inline parent foreign-key resolution for formsets and system checks. | Compare with `django.forms.models._get_foreign_key` when upgrading Django. |
...
| `widget._parse_date_fmt` | `django_veo_admin_api/utils/forms.py` | Expose `SelectDateWidget` ordering metadata without rendering HTML. | Check `SelectDateWidget` date-format parsing on each Django feature release. |
...
- `django.contrib.admin.utils.NestedObjects` in
  `django_veo_admin_api/utils/deletion.py` for protected-delete collection.
- `django.contrib.admin.widgets.url_params_from_lookup_dict` in
  `django_veo_admin_api/admins/base.py` for relation lookup query metadata.
